# Enforce Rate Limiting for Workflow Monitoring Operations: Rate Limiter Instances
...
- **R-RATE-001** MUST: Rate limiter instances MUST be initialized with explicit tokensPerInterval and interval parameters that align with the documented quota limits of the target external service.
...
grep -r "tokensPerInterval" --include="*.js" --include="*.ts" --include="*.py" --include="*.java" . | grep -i "workflow\|monitor"
...
find . -name "*.test.js" -o -name "*.test.ts" -o -name "*.spec.js" -o -name "*.spec.ts" | xargs grep -l "rate.*limit\|workflow.*init" | head -5
```
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Rate limiter instantiation with explicit configuration parameters MUST be confirmed before approving changes to workflow monitoring operations that interact with external services.
</enforcement>