...
- **R-ASYNC-001** MUST: Server page components that accept search parameters MUST declare the searchParams prop with a Promise type signature and await its resolution before accessing parameter values.
...
$test_cmd -- --testPathPattern="integration|search.*param"
...
$lint_cmd "src/**/*.server.{ts,tsx}" --rule "@typescript-eslint/no-floating-promises: error"
```
