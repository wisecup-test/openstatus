# Standardize Context-Aware Structured Logging for Error Reporting: Services Validate Presence
...
- **R-LOGGING-001** SHOULD: Services SHOULD validate the presence of required environment variables at startup and fail fast if critical configuration is missing.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Violations block pull request merge until resolved. Code review findings require remediation before approval. Production incidents caused by inadequate error logging trigger retrospectives and pattern reinforcement.
</enforcement>