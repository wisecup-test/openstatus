...
- **R-TOKEN-001** MUST: Token verification MUST use constant-time comparison functions provided by the hashing library to prevent timing attacks.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All three acceptance criteria must be confirmed before approving code that implements or modifies API key credential storage, token generation, or authentication verification flows.
</enforcement>