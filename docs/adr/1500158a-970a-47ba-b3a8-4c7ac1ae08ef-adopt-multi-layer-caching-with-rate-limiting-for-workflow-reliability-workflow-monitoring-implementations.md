...
1. MUST: Workflow monitoring implementations MUST use a two-tier caching architecture with in-memory cache for hot data and distributed cache for persistent state

## Policy Block

- MUST Workflow monitoring implementations MUST use a two-tier caching architecture with in-memory cache for hot data and distributed cache for persistent state
