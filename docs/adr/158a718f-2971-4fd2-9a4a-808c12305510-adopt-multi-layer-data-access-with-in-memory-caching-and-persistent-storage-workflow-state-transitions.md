...
1. MUST: All workflow state transitions that govern task scheduling or execution flow must persist to durable storage before acknowledging the state change to upstream callers

## Policy Block

- MUST All workflow state transitions that govern task scheduling or execution flow must persist to durable storage before acknowledging the state change to upstream callers
