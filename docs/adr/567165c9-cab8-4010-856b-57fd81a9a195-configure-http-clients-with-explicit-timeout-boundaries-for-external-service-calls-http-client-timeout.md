...
1. SHOULD: HTTP client timeout values SHOULD be coordinated with retry backoff maximum elapsed time to prevent timeout-retry interaction deadlocks

## Policy Block

- SHOULD HTTP client timeout values SHOULD be coordinated with retry backoff maximum elapsed time to prevent timeout-retry interaction deadlocks
