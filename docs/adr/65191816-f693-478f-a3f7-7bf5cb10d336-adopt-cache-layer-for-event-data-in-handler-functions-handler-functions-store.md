...
1. MUST: Handler functions MUST store event data in the cache layer using Set operations when event metadata is computed or retrieved from upstream sources

## Policy Block

- MUST Handler functions MUST store event data in the cache layer using Set operations when event metadata is computed or retrieved from upstream sources
