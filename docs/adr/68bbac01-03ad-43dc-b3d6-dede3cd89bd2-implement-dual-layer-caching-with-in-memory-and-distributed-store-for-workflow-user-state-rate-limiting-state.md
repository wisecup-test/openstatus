...
1. SHOULD: Rate limiting state SHOULD be stored in distributed cache with expiration matching the rate limit window to enable automatic cleanup

## Policy Block

- SHOULD Rate limiting state SHOULD be stored in distributed cache with expiration matching the rate limit window to enable automatic cleanup
