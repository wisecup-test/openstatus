...
1. MUST: In-memory cache structures SHALL be populated from database queries and used for subsequent lookups within the same execution context

## Policy Block

- MUST In-memory cache structures SHALL be populated from database queries and used for subsequent lookups within the same execution context
