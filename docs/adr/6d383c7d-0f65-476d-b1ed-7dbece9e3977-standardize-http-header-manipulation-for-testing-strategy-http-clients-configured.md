...
1. MUST: HTTP clients MUST be configured with explicit timeout values to prevent indefinite blocking during external service checks

## Policy Block

- MUST HTTP clients MUST be configured with explicit timeout values to prevent indefinite blocking during external service checks
