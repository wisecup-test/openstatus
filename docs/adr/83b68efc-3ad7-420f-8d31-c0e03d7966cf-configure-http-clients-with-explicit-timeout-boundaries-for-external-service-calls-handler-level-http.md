...
1. SHOULD: Handler-level HTTP clients SHOULD use request-specific timeout parameters when available, falling back to service-level defaults

## Policy Block

- SHOULD Handler-level HTTP clients SHOULD use request-specific timeout parameters when available, falling back to service-level defaults
