...
1. SHOULD: API key usage tracking SHOULD update last-used timestamps only when sufficient time has elapsed since the previous update to minimize write operations.

## Policy Block

- SHOULD API key usage tracking SHOULD update last-used timestamps only when sufficient time has elapsed since the previous update to minimize write operations.
