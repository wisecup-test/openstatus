...
1. MUST: All HTTP request handlers MUST retrieve event context from the request cache using a consistent key accessor pattern before processing requests

## Policy Block

- MUST All HTTP request handlers MUST retrieve event context from the request cache using a consistent key accessor pattern before processing requests
