...
1. MUST: Temporary interaction state must be stored in a cache layer with time-to-live expiration and consumed atomically after successful processing to prevent duplicate handling

## Policy Block

- MUST Temporary interaction state must be stored in a cache layer with time-to-live expiration and consumed atomically after successful processing to prevent duplicate handling
