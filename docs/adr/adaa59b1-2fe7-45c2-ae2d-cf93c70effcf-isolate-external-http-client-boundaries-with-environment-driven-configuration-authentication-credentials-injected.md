...
1. MUST: Authentication credentials MUST be injected via request headers using bearer token patterns when communicating with authenticated external services

## Policy Block

- MUST Authentication credentials MUST be injected via request headers using bearer token patterns when communicating with authenticated external services
