...
1. MUST: Cache entries for user-scoped workflow data must include expiration windows aligned with the longest workflow step duration to prevent stale reads

## Policy Block

- MUST Cache entries for user-scoped workflow data must include expiration windows aligned with the longest workflow step duration to prevent stale reads
