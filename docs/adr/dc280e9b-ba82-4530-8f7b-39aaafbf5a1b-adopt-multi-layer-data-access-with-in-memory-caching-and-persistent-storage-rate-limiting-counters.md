...
1. MUST: Rate limiting counters and short-lived session state must use in-memory cache storage with explicit time-to-live expiration policies

## Policy Block

- MUST Rate limiting counters and short-lived session state must use in-memory cache storage with explicit time-to-live expiration policies
