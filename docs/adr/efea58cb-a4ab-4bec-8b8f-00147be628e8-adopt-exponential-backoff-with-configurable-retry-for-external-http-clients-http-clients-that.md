...
1. MUST: All HTTP clients that invoke external endpoints for monitoring, health-check, or assertion evaluation operations must wrap the invocation in an exponential backoff retry mechanism.

## Policy Block

- MUST All HTTP clients that invoke external endpoints for monitoring, health-check, or assertion evaluation operations must wrap the invocation in an exponential backoff retry mechanism.
