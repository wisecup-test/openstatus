...
1. SHOULD: Services SHOULD validate that required environment variables are present and non-empty during initialization and fail fast with descriptive error messages if missing

## Policy Block

- SHOULD Services SHOULD validate that required environment variables are present and non-empty during initialization and fail fast with descriptive error messages if missing
